...
if len(sys.argv) != 4:
    ...
    sys.stderr.write(
        "\tpython featurization.py data-dir-path features-dir-path pipeline-pickle-path\n"
    )
    sys.exit(1)
...
def get_df(data):
    df = pd.read_csv(
        data, encoding="utf-8", header=None, delimiter="\t", names=["id", "label", "text"]
    )
...
# Export pipeline pickle
with open(sys.argv[3], "wb") as fd:
    pickle.dump(pipeline, fd, pickle.HIGHEST_PROTOCOL)
